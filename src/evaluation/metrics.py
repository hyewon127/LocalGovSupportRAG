...
# "[십백천]?[만억]"을 따로 둔 이유: 공고문에는 "6백만원", "3천만원"처럼 숫자와 한글 단위를 섞은 표기가 많은데,
# 처음 만든 패턴(억원/만원/원...)은 이걸 못 잡았습니다 - tests/test_answer_eval.py의 가짜 LLM 답변
# "지원금액이 6백만원입니다"에서 발견. 못 잡으면 이런 표기로 지어낸 금액은 검사를 그냥 통과합니다.
_FACT_NUMBER = re.compile(r"\d[\d,.]*\s*(?:[십백천]?\s*[만억]\s*원|[십백천]?\s*[만억]|천\s*원|원|%|개사|개월|명|년|일)")
